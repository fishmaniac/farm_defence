...
pub fn check_player_collisions(player: &mut player_manager::PlayerManager, game: &mut game_manager::GameManager, events: &mut event_manager::EventManager, new_position: (i32, i32), level: &mut level_manager::LevelManager) -> bool {
    ...
    let new_rect = sdl2::rect::Rect::new(centered_new_x - player.x + new_offset, centered_new_y - player.y + new_offset, tile_size_offset as u32, tile_size_offset as u32);
...
pub fn check_enemy_collisions (game: &mut game_manager::GameManager, events: &mut event_manager::EventManager, new_position: (u32, u32), level: &mut level_manager::LevelManager) -> bool {
    let mut colliding = false;
    //shouldnt have to make the box this small
    let tile_size_offset = constants::TILE_SIZE as i32/*  / 16 */;

    //allows for rect collisions but might be slower
    let new_rect = sdl2::rect::Rect::new(new_position.0 as i32 - game.cam_x, new_position.1 as i32 - game.cam_y, tile_size_offset as u32, tile_size_offset as u32);
    for col_index in 0..level.level_vec.len() {
        for row_index in 0..level.level_vec[col_index].len() {
            let temp_tile = &mut level.level_vec[col_index][row_index];
