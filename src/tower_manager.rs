...
impl TowerManager {
    ...
    pub fn check_attacks(&mut self, game: &mut game_manager::GameManager, level: &mut level_manager::LevelManager, enemies: &mut enemy_manager::EnemyManager,
        health_bars: &mut gui_manager::GUIManager,) {

        let mut tower_index = 0;

        while tower_index < self.tower_vec.len() {
            let tower_pos = (self.tower_vec[tower_index].bottom_col_index as i32, self.tower_vec[tower_index].bottom_row_index as i32);

            let mut enemy_index = 0;
            while enemy_index < enemies.enemy_vec.len() {
                let enemy_pos = (enemies.enemy_vec[enemy_index].col_index as i32, enemies.enemy_vec[enemy_index].row_index as i32);

                //TOWER ATTACK
                if is_within_radius(tower_pos, enemy_pos, self.tower_vec[tower_index].attack_radius) {
                ...
                //ENEMY ATTACK
                if is_within_radius(tower_pos, enemy_pos, enemies.enemy_vec[enemy_index].attack_radius) {
                    if self.tower_vec[tower_index].health != 0 {
                        ...
                        while target_index < game.target_vec.len() {
                            if is_within_radius((game.target_vec[target_index].0 as i32, game.target_vec[target_index].1 as i32), enemy_pos, 3) {
        ...
        fn is_within_radius(tower_pos: (i32, i32), enemy_pos: (i32, i32), radius: i32) -> bool {
