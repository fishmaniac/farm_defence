...
impl<'a> UpgradeManager <'a> {
        ...
        if !upgrade_exists {
            match temp_tile.tile_type {
                constants::TILE_TYPE_ARCHER_BOTTOM => {
                    self.create_upgrades(gui_manager, towers, BuildingType::Archer, grid_index);
                },
                constants::TILE_TYPE_FIREBALL_BOTTOM => {
                    self.create_upgrades(gui_manager, towers, BuildingType::Fireball, grid_index);
                },
                _ => {},
            }
        }
            ...
            BuildingType::Archer => {
                for tower_index in 0..towers.tower_vec.len() {
                    ...
                    if tower.bottom_index == grid_index {
                        let upgrade_menu: Option<UpgradeMenu> = match Self::initialize_upgrade_menu(grid_index, tower_index) {
                            Ok(upgrade_menu) => Some(upgrade_menu),
                            Err(e) => None,
                        };
                        match upgrade_menu {
                            Some(mut upgrade_menu) => {
                                if let Ok(upgrade_first_0) = self.create_upgrade_button("broadheads", 0, constants::TOWER_ARCHER_DAMAGE * 2, 0, 5) {
                                    upgrade_menu.upgrades_first_path.push_back(upgrade_first_0);
                                }
                                if let Ok(upgrade_first_1) = self.create_upgrade_button("extra training", 0, constants::TOWER_ARCHER_DAMAGE * 3, 5, 1) {
                                    upgrade_menu.upgrades_first_path.push_back(upgrade_first_1);
                                }
                                if let Ok(upgrade_second_0) = self.create_upgrade_button("reinforced tower", constants::TOWER_ARCHER_HEALTH * 2, 0, 0, 5) {
                                    upgrade_menu.upgrades_second_path.push_back(upgrade_second_0);
                                }
                                if let Ok(upgrade_second_1) = self.create_upgrade_button("more reinforceteststststststststststst", constants::TOWER_ARCHER_HEALTH * 3, 0, 0, 15) {
                            ...
                            None => gui_manager.create_message("error creating upgrade... why".to_string(), 1028),
                        }
                    }
                }
            },
            BuildingType::Fireball => {
                for tower_index in 0..towers.tower_vec.len() {
                    ...
                    if tower.bottom_index == grid_index {
                        ...
                        match upgrade_menu {
                            Some(mut upgrade_menu) => { 
                                if let Ok(upgrade_first_0) = self.create_upgrade_button("bigger balls", 0, constants::TOWER_FIREBALL_DAMAGE * 2, 5, 5) {
                                    upgrade_menu.upgrades_first_path.push_back(upgrade_first_0);
                                }
                                if let Ok(upgrade_first_1) = self.create_upgrade_button("extra hot balls", 0, constants::TOWER_FIREBALL_DAMAGE * 3, 0, 1) {
                                    upgrade_menu.upgrades_first_path.push_back(upgrade_first_1);
                                }
                                if let Ok(upgrade_second_0) = self.create_upgrade_button("reinforced tower", constants::TOWER_FIREBALL_HEALTH * 2, 0, 0, 5) {
                                    upgrade_menu.upgrades_second_path.push_back(upgrade_second_0);
                                }
                                if let Ok(upgrade_second_1) = self.create_upgrade_button("m", constants::TOWER_FIREBALL_HEALTH * 3, 0, 0, 15) {
                                    upgrade_menu.upgrades_second_path.push_back(upgrade_second_1);
                                }
    ...
    -> Result<UpgradeMenu<'a>, String> {
        let upgrade_menu = UpgradeMenu {
            background_rect: sdl2::rect::Rect::new(0, 0, constants::TILE_SIZE, constants::TILE_SIZE * 2),
    ...
    -> Result<Upgrade<'a>, String> {
        let texture_surface = self.font.render(&format!("{} {}", cost, text).to_string())
        ...
        if !game.build_mode && !game.seed_mode {
            for upgrade_index in  0..self.upgrade_menu_vec.len() {
                ...
                if upgrade.current_first_path.is_none() {
                    if let Some(current_first) = upgrade.upgrades_first_path.pop_front() {
                        upgrade.current_first_path = Some(current_first);
                    }
                }
                if upgrade.current_second_path.is_none() {
                    if let Some(current_second) = upgrade.upgrades_second_path.pop_front() {
                ...
                if upgrade.menu_active {
                    if let Some(current_first) = &mut upgrade.current_first_path {
                        ...
                        && game.gold_amount >= current_first.cost as u32 {
                            Self::upgrade_current_upgrade(game, towers, current_first, upgrade_index);
...
/*                             println!("PREV DAMAGE: {}, PREV RADIUS: {}, UPGRADE DAMAGE: {}, UPGRADE_RADIUS: {}", towers.tower_vec[upgrade_index].projectile_damage, towers.tower_vec[upgrade.building_index].attack_radius, current_first.damage, current_first.radius);  */
                    ...
                    if let Some(current_second) = &mut upgrade.current_second_path {
                        ...
                        && game.gold_amount >= current_second.cost as u32 {
                            Self::upgrade_current_upgrade(game, towers, current_second, upgrade_index);
                            upgrade.background_rect.set_width(0);
            ...
            for upgrade in &mut self.upgrade_menu_vec {
                if upgrade.menu_active {
                    ...
                    if let Some(upgrades) = &mut upgrade.current_second_path {
                        upgrades.upgrade_rect.set_x(upgrade.grid_index.0 as i32 * constants::TILE_SIZE as i32 - game.cam_x); 
                        upgrades.upgrade_rect.set_y(upgrade.grid_index.1 as i32 * constants::TILE_SIZE as i32 + upgrades.texture_surface.height() as i32 - game.cam_y);
                        upgrades.upgrade_rect.set_width(upgrades.texture_surface.width());
                        upgrades.upgrade_rect.set_height(upgrades.texture_surface.height());
                        if upgrades.upgrade_rect.width() > upgrade.background_rect.width() {
                            upgrade.background_rect.set_width(upgrades.upgrade_rect.width());
